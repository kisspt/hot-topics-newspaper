...
导航分两套呈现，靠断点切换，**一次只挂一份**（JS 媒体查询，不是把两套都塞进 DOM）：

- **窄屏（md: 以下）**：底部三个 Tab —— **速记**（默认落地页）、**流水**、**未收款**（红点角标显示笔数）。
- **宽屏（md: 起）**：导航移到**左侧边栏**，顶栏保留品牌、同步圆点与退出。

> 桌面版是 2026-09-16 追加的范围（原 spec 只写了底部 Tab）。同一次追加还把两个列表屏在宽屏上改成表格、速记屏在宽屏上放宽容器；理由与逐条取舍见实现计划的「桌面版」一节。
...
按日期分组倒序。窄屏每条一张卡片；宽屏换成表格（同样的分组表头 + 倒序），行右侧常驻「收款 / 完成 / 删除」。

> 「左滑快捷操作」只在窄屏成立。宽屏没有左滑，三个动作以常驻按钮呈现——这同时修掉了窄屏上「动作藏在卡片底下、键盘到得了但焦点看不见」的问题。
