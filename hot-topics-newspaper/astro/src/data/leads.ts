/**
 * Lead / body text generators — same spirit as the Python build.py
 * but implemented in TypeScript for Astro's build pipeline.
 */
...
export function shortLead(idx: number): string {
  return LEADS[(idx + 1) % LEADS.length];
}

export function featureBody(idx: number): string {
  return `${LEADS[idx % LEADS.length]}${LEADS[(idx + 3) % LEADS.length]}`;
}

export function featureBodyLong(idx: number): string {
  return `${LEADS[idx % LEADS.length]}${LEADS[(idx + 3) % LEADS.length]}${LEADS[(idx + 5) % LEADS.length]}`;
}
